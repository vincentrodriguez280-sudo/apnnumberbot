    ...
    for i in range(18):
        ...
        try:
            ...
            if otp:
                ...
                elif "MADAGASCAR" in country_code.upper(): country_short = "MG"
                elif "NEPAL" in country_code.upper(): country_short = "NP"
                elif "USA" in country_code.upper(): country_short = "US"
                ...
                try:
                    kb_group = [
                        ...
                        [InlineKeyboardButton(f"📞 Get Number", url=cfg["number_bot_url"])]
                    ]
                except:
                    kb_group = [
                        [InlineKeyboardButton(f"🎉 Channel", url=cfg["community_url"]), InlineKeyboardButton(f"🔑 {otp}", callback_data=f"copy_{otp}")],
                        [InlineKeyboardButton(f"📞 Get Number", url=cfg["number_bot_url"])]
                    ]
                try:
                    await bot.send_message(chat_id=cfg["otp_group_id"], text=text_group, reply_markup=InlineKeyboardMarkup(kb_group))
                except Exception as e:
                    print(f"[GROUP SEND ERR] {e}")
                    try:
